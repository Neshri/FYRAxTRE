...
from wiktionary import get_wiktionary_senses, get_base_lemma_senses, WIKTIONARY_USER_AGENT


def build_word_inventory(
    lexicon: dict,
) -> tuple[dict[str, dict[str, str]], dict[str, str], dict[str, dict[str, str | list[str]]]]:
    """
    ...
    Homographs: the lexicon can have two different senses sharing the same
    (POS, baseform) -- e.g. "rev" (nn) meaning both "reef" and "fishing
    line". Picking "whichever definition was seen first" would silently
    attach the WRONG one to a word suggested for a specific, different sense
    -- worse than not resolving it, since it looks plausible instead of
    failing loudly. So each definitions_by_pos value is either a plain str
    (one sense seen, unambiguous) or a list[str] of every DISTINCT
    definition seen for that (POS, baseform) once a second one shows up.
    Callers that hit a list can't just pick one -- see disambiguate_homographs(),
    which asks the model to choose using the category's own context, rather
    than the lookup silently guessing.
    """
    inventory_by_pos = {}
    all_words_map = {}
    definitions_by_pos = {}
    for entry in lexicon.values():
        ...
        if bf:
            ...
            if pos:
                ...
                pos_defs = definitions_by_pos.setdefault(pos, {})
                existing = pos_defs.get(bf_lower)
                if existing is None:
                    pos_defs[bf_lower] = definition
                elif isinstance(existing, list):
                    if definition not in existing:
                        existing.append(definition)
                elif existing != definition:
                    pos_defs[bf_lower] = [existing, definition]
...
6. FÖR FRÄMMANDE ELLER OBSKYR BETYDELSE:
   Listan nedan är hämtad från lexikon och Wiktionary, och Wiktionary-sidor är organiserade per
   stavning över ALLA språk -- en "betydelse" kan därför egentligen vara ett annat språks ord som
   råkar stavas likadant (t.ex. ett hebreiskt månadsnamn "Av", inte svenska prepositionen "av"),
   inte en verklig svensk betydelse alls. Andra betydelser kan vara äkta svenska men begränsade
   till en så smal religiös, teknisk eller specialiserad kontext att de flesta svensktalande aldrig
   skulle känna igen dem. Om definitionen beskriver något sådant, markera betydelsen som "unusable"
   med anledningen "för främmande/obskyr för en allmän pusselspelare" -- även om kandidatlistan
   råkar innehålla ord som passar rent lexikalt mot den definitionen.
   Var INTE överförsiktig här: en betydelse som bara är lite formell, ålderdomlig i vardagligt tal,
   eller mindre vanlig räknas INTE som obskyr på det här sättet -- bara sådant en genomsnittlig
   vuxen svensktalande sannolikt aldrig skulle stöta på eller förstå.

7. INGA DUBBLETTER INOM DETTA SVAR:
...
def resolve_sense_category_pool(word, raw_categories, sense_reports, args, reject_log):
    ...
    disambiguate_homographs(word, candidate_categories, args)
...
def build_disambiguation_prompt(word, ambiguous_items):
    """
    ambiguous_items: list of dicts with category_label, category_definition,
    reasoning, sibling_word, options (list of distinct dictionary definitions
    that share the word's spelling + part of speech). Asks the model to pick
    which sense was actually meant, using the category's own context -- the
    same context it used when it chose the word in stage 1.
    """
    lines = []
    lines.append(f"Pivotord: \"{word}\" (svenska)")
    lines.append("Följande ord föreslogs som syskonord, men lexikonet har FLERA olika betydelser "
                 "för samma stavning och ordklass -- riktiga homonymer (t.ex. \"rev\" kan betyda "
                 "både 'korallrev' och 'metrev'). Avgör utifrån kategorins sammanhang nedan vilken "
                 "betydelse som avsågs.")
    lines.append("")
    for i, item in enumerate(ambiguous_items, 1):
        lines.append(f"--- ORD {i}: {item['sibling_word']} ---")
        lines.append(f"Kategori: {item['category_label']} (pivotordets betydelse här: {item['category_definition']})")
        lines.append(f"Varför ordet föreslogs: {item['reasoning']}")
        lines.append("Möjliga betydelser för detta ord:")
        for j, opt in enumerate(item["options"], 1):
            lines.append(f"  {j}. {opt}")
        lines.append("")
    lines.append("""Avsluta ditt svar med EXAKT ETT JSON-kodblock (och inget annat efter det), en post per ord ovan i samma ordning:
```json
{
  "choices": [
    {"word": "...", "chosen_index": N}
...
```
Om inget alternativ passar kategorin -- ordet dög inte trots allt -- sätt "chosen_index": null för det ordet.""")
    return "\n".join(lines)


def disambiguate_homographs(word, categories, args):
    """
    Finds every sibling verify_and_correct_sibling couldn't resolve to a
    single sense (sib['definition_options'] set -- a real Swedish homograph,
    same spelling + POS, multiple distinct dictionary senses), and asks the
    model once, in a single batched call covering every such word for this
    whole pivot, to pick the sense that matches each category's own context.
    Skips the call entirely if no homograph occurred (the common case) --
    this doesn't add cost to pivots that don't need it.

    On a clean pick, sets sib['definition'] and drops definition_options.
    On a declined/unparseable/out-of-range answer, leaves definition_options
    in place and definition empty -- the existing missing_definition gate in
    filter_valid_categories() still catches it, so a bad answer here fails
    safe rather than shipping a guess.
    """
    ambiguous_items = []
    for cat in categories:
        for sib in cat.get("siblings", []):
            opts = sib.get("definition_options")
            if opts:
                ambiguous_items.append({
                    "category_label": cat.get("category_label", ""),
                    "category_definition": cat.get("definition", ""),
                    "reasoning": cat.get("reasoning", ""),
                    "sibling_word": sib.get("word", ""),
                    "options": opts,
                    "_sib": sib,
                })
    ...
    prompt = build_disambiguation_prompt(word, ambiguous_items)
    print(f"  [disambiguation] '{word}': {len(ambiguous_items)} homograph(s) need a sense picked "
    ...
    try:
        ...
        print(f"  [disambiguation] '{word}': could not parse response -- leaving homograph(s) unresolved.")
        return

    # Queue per word rather than a flat dict, in case the same spelling is
    # ambiguous in more than one category in this same response.
    choices_by_word = {}
    for c in result.get("choices", []):
        choices_by_word.setdefault(c.get("word", "").strip().lower(), []).append(c)

    for item in ambiguous_items:
        sib = item["_sib"]
        queue = choices_by_word.get(item["sibling_word"].strip().lower(), [])
        choice = queue.pop(0) if queue else None
        idx = choice.get("chosen_index") if choice else None
        if isinstance(idx, int) and 1 <= idx <= len(item["options"]):
            sib["definition"] = item["options"][idx - 1]
            sib.pop("definition_options", None)
        else:
            print(f"  [disambiguation] '{word}': no usable choice for '{item['sibling_word']}' "
                  f"-- its category will be dropped if this isn't resolved.")
